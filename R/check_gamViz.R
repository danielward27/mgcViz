...
check.gamViz <- function(
  ...
  type <- match.arg(type)
  if (type == "auto") {
    type <- .getResTypeAndMethod(obj$family)$type
  }
  ...
  for (nam in names(a.all)) {
    assign(
      nam,
      .argSetup(a.all[[nam]], get(nam), nam, verbose = FALSE),
      envir = environment()
    )
  }
  ...
  df <- data.frame(linpred = linpred, resid = resid, response = resp, fv = fv)
  dfS <- df[subS, ]
  ...
  plots[[3]] <-
    ...
    labs(title = "Histogram of residuals", xlab = "Residuals")
  plots[[4]] <-
  ...
  if (
    (obj$method %in%
      c("GCV", "GACV", "UBRE", "REML", "ML", "P-ML", "P-REML", "fREML"))
  ) {
    ...
    if (!is.null(obj$outer.info)) {
      if (obj$optimizer[2] %in% c("newton", "bfgs")) {
        ...
        if (
          is.null(obj$family$available.derivs) ||
            obj$family$available.derivs > 0
        ) {
          cat(
            "\nGradient range [",
            min(boi$grad),
      ...
      if (length(obj$sp) == 0)
        ...
        if (!is.null(obj$mgcv.conv)) {
          ...
          cat(
            "The RMS",
            obj$method,
            "score gradient at convergence was",
            obj$mgcv.conv$rms.grad,
            ".\n"
          )
          if (obj$mgcv.conv$hess.pos.def)
            cat("The Hessian was positive definite.\n") else
            cat("The Hessian was not positive definite.\n")
        }
      }
    }
    if (!is.null(obj$rank)) {

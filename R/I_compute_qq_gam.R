...
.compute.qq.gam <- function(o, type, method, CI, level = level, rep, sortFun) {
  ...
  if (
    o$method %in%
      c("PQL", "lme.ML", "lme.REML", "lmer.REML", "lmer.ML", "glmer.ML")
  ) {
    method <- "normal"
  }
  out <- NULL
  if (method %in% c("simul1", "simul2")) {
    out <- .compute.qq.gam.simul(
      o = o,
      type = type,
      method = method,
      CI = CI,
  ...
  if (method %in% c("tnormal", "tunif")) {
    out <- .compute.qq.gam.tunif.tnormal(
      o = o,
      method = method,
      CI = CI,
      level = level,
...
.compute.qq.gam.simul <- function(o, type, method, CI, level, rep, sortFun) {
  ...
  if (method == "simul2") {
    ...
    U <- (ix - 0.5) / n
    if (!is.null(fam$qf)) {
      # If CDF not available use first method
      dm <- list()
      indx <- 1:n
      for (ii in 1:rep) {
  ...
  if (!is.null(Dq)) {
    ...
    if (CI == "quantile") {
      conf <- rbind(
        rowOrderStats(dm, which = ceiling(rep * alpha)),
...
.compute.qq.gam.tunif.tnormal <- function(o, method, CI, level, rep, sortFun) {
  ...
  if (!is.null(fam$cdf)) {
    ...
    if (CI != "none") {
      alpha <- (1 - level) / 2
      if (method == "tnormal") {
        # Normal CI (from "Worm plot: a simple diagnostic device
        # for modelling growth reference curves") or ...
        tmp <- qnorm(alpha) * sqrt(p * (1 - p) / n) / dnorm(Dq)
        conf <- rbind(Dq + tmp, Dq - tmp)
      } else {
        # ... uniform CI (from distribution of uniform order stats (see Wikip))
        tmp <- 1:n
        conf <- rbind(
          qbeta(alpha, tmp, n - tmp + 1),
  ...
  ylab <- ifelse(
    method == "tnormal",
    "Obs y transf to normal",
    "Obs y transf to unif"
  )
  return(list(
    "D" = D,
    "Dq" = Dq,
...
.compute.qq.gam.normal <- function(o, type, method, CI, level, sortFun) {
  ...
  return(list(
    ...
    "conf" = conf,
    "method" = method,
    "ylab" = paste("normalized", type, "residuals")
  ))
}

...
simulate.gam <- function(
  ...
  if (missing(newdata)) {
    # (a) the offset should already be included in o$fitted.values
    ...
    if (!is.null(offset)) {
      message(
        "simulate.gam: offset argument ignored. No newdata provided, so offset is already in object$fitted.values"
      )
    }
  } else {
    # (b) the user-defined offset is added to linear predictor
    ...
    if (is.list(form)) {
      ...
      mu <- t(laply(1:nte, function(.ii) {
        lnki[[.ii]](mu[, .ii] + offset[[.ii]])
      }))
    } else {
      # [2] GAM case
      if (is.null(offset)) {
  ...
  if (o$family$family == "Multivariate normal") {
    out <- rlply(
      nsim,
      function(.nouse) rmvn(nrow(mu), mu, solve(crossprod(o$family$data$R)))
    )
    return(out)
  }
  if (
    grepl(
      "Multivariate normal - Covariance modelling via",
      o$family$family,
      fixed = TRUE
    )
  ) {
    out <- rlply(
      nsim,
      function(.nouse) o$family$rd(mu = mu, wt = NULL, scale = NULL)
    )
    return(out)
  }
...
.simulate.gam <- function(mu, w, sig, method, fam, nsim, u) {
  ...
  if (method == "auto") {
    ...
    if (method == "qf" && is.null(fam$qf)) {
      stop("No simulation method available for this family")
    }
  }
  ...
  if (method == "qf") {
    ...
    if (is.null(fam$qf)) {
      stop("fam$qf unavailable, try using method = `rd`")
    }

...
.prepare.ALE <- function(
  ...
  .out <- structure(
    list("ALE" = .data, "type" = .cls, "xval" = .x, "xnam" = xnam),
    class = "ALE1D"
  )
  return(.out)
}
...
.prepare.ALE.factor <- function(
  ...
  Dfx2 <- lapply(
    ...
    function(.ii)
      colSums(
        jacFun(.o = o, .d = X2[X2[[xnam]] == x[.ii + 1], ], .t = type, ...)$J -
          jacFun(
            .o = o,
            .d = data2[data2[[xnam]] == x[.ii], ],
            .t = type,
            ...
          )$J
      )
  ...
  if (center == 1) {
    # ... by substracting E(fx), as in Apley's paper
  ...
  if (center == 2) {
    ...
    Dfx <- t(
      t(Dfx) +
        colMeans(
          jacFun(.o = o, .d = data[data[[xnam]] == x[1], ], .t = type, ...)$J
        )
    )
  }
  ...
  V <- Dfx %*%
    varFun(
      .o = o,
      .t = type,
      .J = jacFun(.o = o, .d = X2[1:2, ], .t = type, ...),
      ...
    ) %*%
    t(Dfx)

  out <- list(
    "ALE" = data.frame(x = x, y = fx, se = sqrt(diag(V))),
    "varALE" = V
  )
...
.prepare.ALE.numeric <- function(
  ...
  Dfx <- lapply(
    ...
    function(.bn)
      colMeans(
        jacFun(.o = o, .d = X2[bnum == .bn, ], .t = type, ...)$J -
  ...
  if (center == 1) {
    ...
    Dfx <- t(
      t(Dfx) - colSums((Dfx[1:K, ] + Dfx[2:(K + 1), ]) / 2 * b1) / sum(b1)
    )
  }
  if (center == 2) {
    # ... by adding E(mu(z_0))
    ...
    Dfx <- t(
      t(Dfx) + colMeans(jacFun(.o = o, .d = X1[bnum == 1, ], .t = type, ...)$J)
    )
  }
  ...
  out <- list(
    "ALE" = data.frame(x = z, y = fx, se = sqrt(diag(V))),
    "varALE" = V
  )

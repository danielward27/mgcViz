...
check0D <- function(
  ...
  if (dy > 1 && is.null(trans)) {
    message(
      "Response y is vector-valued, using trans <- function(y) drop(rowSums(y)) to reduce it to a vector"
    )
  ...
  pl <- ggplot(data = res, mapping = aes(x = x)) +
    theme_bw() +
    theme(
  ...
  out <- structure(
    list(
      ...
      "type" = c(
        "Check",
        "0D",
        ifelse(length(y) == 1, "Scalar", "Vector"),
        .simpleCap(cls)
      )
    ),

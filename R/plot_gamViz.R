...
plot.gamViz <- function(
    ...
    ...) {
  if (!inherits(x, "gam")) {
    stop("Argument 'x' should be of class 'gam' or 'gamViz'. See ?getViz")
  }

  if (!inherits(x, "gamViz")) {
    x <- getViz(x, ...)
  }
  ...
  if (!is.null(smo)) {
    ...
    wrap <- function(.smo, .n, .n2, ...) {
      if ("mgcv.smooth.MD" %in% class(.smo)) {
        return(NULL)
      }
      if ("mgcv.smooth.2D" %in% class(.smo)) {
        .n <- .n2
      }

...
.plot.multi.mgcv.smooth.1D <- function(P, trans, asFact, isMQGAM) {
  ...
  .dat <- list()
  .dat$fit <- data.frame(
    "x" = rep(.fitDat[[1]]$x, length(.fitDat)),
  ...
  if (!isMQGAM) {
    .dat$fit$id <- if (is.null(asFact) || asFact) {
      as.factor(.dat$fit$id)
    } else {
      as.numeric(.dat$fit$id)
    }
  }
  ...
  if (isMQGAM) {
    ...
    if (is.null(asFact)) {
      asFact <- length(.idNam) < 10
    }
    if (asFact == TRUE) {
      .pl <- .pl +
        scale_color_gradient(
          breaks = round(sort(.idNam, decreasing = TRUE), 3)
        ) +
      ...
      if (min(diff(sort(.idNam))) > 0.099) {
        ...
        .pl <- .pl +
          scale_color_gradient(breaks = sort(.idNam, decreasing = TRUE))
      }
    }
  }

...
plotSlice <- function(x, fix, a.facet = list(), ...) {
  ...
  if (grD > 2) {
    stop(
      "'fix' cannot contain more than 2 vectors. Some variables must be fixed to scalars."
    )
  }
  if (grD == 0) {
  ...
  plts <- alply(
    ...
    function(.vr, ...) {
      ...
      .d$fit[paste0(".fx.", names(.vr))] <- drop(matrix(
        rep(.vr, each = nrow(.d$fit)),
        nrow(.d$fit),
        nfx
      ))
      .d$res[paste0(".fx.", names(.vr))] <- drop(matrix(
        rep(.vr, each = nrow(.d$res)),
        nrow(.d$res),
        nfx
      ))
      return(.d)
  ...
  if (is.null(a.facet$labeller)) {
    a.facet$labeller <- function(labels, multi_line = TRUE, sep = "=") {
  ...
  if (grD == 1) {
    if (is.null(a.facet$nrow) && is.null(a.facet$ncol)) {
      a.facet$ncol <- floor(sqrt(nsl))
    }
    if (is.null(a.facet$facets)) {
      a.facet$facets <- as.formula(paste0("~ .fx.", gridVar))
    }
    .pl <- .pl + do.call("facet_wrap", a.facet)
  }
  if (grD == 2) {
    if (is.null(a.facet$facets)) {
      a.facet$facets <- paste0(".fx.", gridVar[1], " ~ .fx.", gridVar[2])
    }
  ...
  out <- structure(
    list("ggObj" = .pl, "data" = .dat, "type" = c("MD", "slice")),

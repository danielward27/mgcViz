...
addPlotSmooth <- function(e1, e2) {
  ...
  if ("gamLayer" %in% class(e2)) {
    ...
    e2$arg$data <- e1$data
    fun <- tryCatch(
      get(paste(e2$fun, ".", paste(e1$type, collapse = ''), sep = '')),
      error = function(e) {
        ...
        if (grepl("not found", e)) {
          warning(paste(
            "No ",
            e2$fun,
            "() layer available for type \"",
            paste(e1$type, collapse = ' '),
            "\"",
            sep = ''
          ))
...
addPlotGam <- function(e1, e2) {
  ...
  e1$plots <- lapply(e1$plots, function(.l) {
    return(withCallingHandlers(.l + e2, warning = function(w) {

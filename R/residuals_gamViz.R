...
residuals.gamViz <- function(object, type = "deviance", ...) {
  if (!inherits(object, "gamViz")) {
    stop("Argument 'object' should be of class 'gamViz'. See ?getViz")
  }
  ...
  if (type %in% c("tunif", "tnormal")) {
    ...
    if (is.null(fam$cdf)) {
      stop(
        "CDF unavailable for this family: can't use `tunif` or `tnormal` type"
      )
    }

...
check2D <- function(
  ...
  if (is.list(x1) && is.list(x2)) {
    ...
    if (np != length(x2)) {
      message(
        "check2D: length(x1) != length(x2), not all pairs of variables will be plotted"
      )
      np <- min(np, length(x2))
    }
    out <- lapply(
  ...
  tmp <- .getresidualsTransformSubsample(
    o = o,
    y = y,
    maxpo = maxpo,
    trans = trans,
  ...
  if (cls2 == "factor" && cls1 != "factor") {
    message(
      "check2D likes to have factor variables on x axis: I am swapping the coordinates."
    )
    tmp <- x1
    x1 <- x2
    x2 <- tmp
  ...
  pl <- ggplot(data = res, mapping = aes(x = x, y = y, z = z)) +
    theme_bw() +
    theme(
  ...
  if (inherits(o, "qgam")) {
    misc$qu = o$family$getQu()
  }

  out <- structure(
    list(
      "ggObj" = pl,
      "data" = list("res" = res, "sim" = sim, "misc" = misc),

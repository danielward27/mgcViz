...
postSim <- function(
  ...
  if (is.null(attr(X, "lpi"))) {
    ...
    # Multiple linear predictor case
    out <- .postSimMLP(
      o = o,
      X = X,
      cf = cf,
...
.postSim1LP <- function(
  ...
  if (!is.null(offset)) {
    # User provided offset: nothing to do
    offI <- offset
  } else {
    offI <- numeric(n) # Default offset is zero
    if (!is.null(attr(X, "model.offset"))) {
      # Original fit included offset
      if (missing(newdata)) {
        # We re-calculate it using original data
        offset <- muHat - drop(X %*% cf)
      } else {
        # If we are using new data we need the user
...
.postSimMLP <- function(
  ...
  if (!is.null(offset)) {
    ...
    if (!is.null(attr(X, "model.offset"))) {
      # Original fit included offset
      if (missing(newdata)) {
        # We re-calculate it for each term using original data
        offset <- lapply(1:nte, function(.ii) {
  ...
  simY <- llply(1:nsim, function(ii) {
    ...
    mu <- t(laply(
      ...
      function(.ii) {
        lnki[[.ii]](
          X[, lpi[[.ii]], drop = FALSE] %*% beta[lpi[[.ii]]] + offI[[.ii]]
        )
      }
    ))
    # Simulated and transform

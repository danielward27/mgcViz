...
.plot.qq.gam <- function(P, CI, worm, showReps, xlimit, ylimit, a.all, ...) {
  ...
  if (worm) {
    if (CI && !is.null(P$conf)) {
      P$conf$y <- P$conf$y - P$conf$x
    }
    if (showReps && !is.null(P$dm)) {
      P$dm$y <- P$dm$y - P$dm$x
    }
    P$D <- P$D - P$Dq
  }
  ...
  if (showReps && !is.null(P$dm)) {
    # Add a line for each simulation
    .pl <- .pl +
      do.call(
        "geom_line",
  ...
  .pl <- .pl +
    do.call(
      "geom_abline",
      c(list(intercept = 0, slope = !worm), a.all$a.ablin)
    ) +
    do.call(
      "geom_point",
      c(
        list(data = data.frame("sx" = P$Dq, "sy" = P$D), aes(x = sx, y = sy)),
        a.all$a.qqpoi
      )
    ) +
    labs(

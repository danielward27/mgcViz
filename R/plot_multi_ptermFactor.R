...
plot.multi.ptermFactor <- function(x, a.facet = list(), asFact = TRUE, ...) {
  ...
  out <- .plot.multi.ptermFactor(
    P = P,
    trans = identity,
    a.facet = a.facet,
    asFact = asFact,
    isMQGAM = attr(x, "isMQGAM")
  )
...
.plot.multi.ptermFactor <- function(P, trans, a.facet, asFact, isMQGAM) {
  ...
  .dat <- list()
  .dat$fit <- data.frame(
    "x" = as.factor(rep(.fitDat[[1]]$x, length(.fitDat))),
  ...
  if (asFact) {
    .dat$fit$id <- as.factor(.dat$fit$id)
  }
  ...
  .pl <- ggplot(data = .dat$fit, aes("x" = id, "y" = ty)) +
    labs(title = NULL, y = P$ylab) +
    theme_bw() +
    theme(
  ...
  if (isMQGAM) {
    # Setting legend for MQGAMs
    if (asFact) {
      if (is.numeric(.idNam)) {
        .idNam <- round(as.numeric(levels(.dat$fit$id)), 3)
      }
      .pl <- .pl +
        scale_x_discrete(labels = .idNam) +
        scale_colour_discrete(labels = .idNam)
    } else {
      if (min(diff(sort(.idNam))) > 0.099) {
        # Ticks will be plotted if they are more than 10% apart, rounding error prevents >=0.1
  ...
  if (is.null(a.facet$facets)) {
    a.facet$facets <- as.formula("~ x")
  }
  .pl <- .pl + do.call("facet_wrap", a.facet)

  return(structure(
    list("ggObj" = .pl, "data" = .dat, "type" = c("Multi", "Pterm", "Factor")),
